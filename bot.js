...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(userId,
    `Salam, ${name}! 🌙\n\nBu bot sənə həftəlik ibadət tapşırıqları paylayır.\n\n` +
    `Komandalar:\n/mytask - Bu həftəki tapşırığını gör\n/done - Tapşırığı etdim ✅\n/notdone - Tapşırığı etmədim ❌`
  );
});
...
bot.onText(/\/mytask/, (msg) => {
  ...
  if (!data.options.length) {
    return bot.sendMessage(userId, '⚠️ Hələ heç bir tapşırıq əlavə edilməyib.');
  }

  const member = data.members[userId];
  if (member && member.pending.length > 0) {
    const idx = member.pending[member.pending.length - 1];
    return bot.sendMessage(userId, `📋 Bu həftəki tapşırığın:\n\n*${data.options[idx]}*`, { parse_mode: 'Markdown' });
  }

  const userName = msg.from.first_name;
  const optionIndex = assignOption(data, userId, userName);
  saveData(data);
...
bot.onText(/\/listoption/, (msg) => {
  ...
  // Telegram mesaj limiti 4096 simvoldur, uzunsa böl
...
// /send - kanalda hamıya birdən mesaj at
bot.onText(/\/send/, async (msg) => {
  ...
  mesaj += `\nTapşırığı etdikdə ✅ /done, etmədikdə ❌ /notdone yazın (şəxsi mesajda).`;
